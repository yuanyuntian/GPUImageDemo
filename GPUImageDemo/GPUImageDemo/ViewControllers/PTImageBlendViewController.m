...
- (void)viewDidLoad {
    ...
    self.slider4 = [[UISlider alloc] init];
    self.slider4.minimumValue = 0.0;
    self.slider4.hidden = true;
    self.slider4.tag = 3;
    [self.view addSubview:self.slider4];
    [self.slider4 mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.right.equalTo(self.slider1);
        make.top.equalTo(self.slider3.mas_bottom).offset(20);
    }];
    [self.slider4 addTarget:self action:@selector(sliderValueChanged:) forControlEvents:UIControlEventValueChanged];
    [self.slider4 setContinuous:true];
