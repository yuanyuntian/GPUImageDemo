...
static OSStatus aacEncodeInputDataProc(AudioConverterRef inAudioConverter, UInt32 *ioNumberDataPackets, AudioBufferList *ioData, AudioStreamPacketDescription **outDataPacketDescription, void *inUserData){
    PTHAACEncoder *hwAacEncoder = (__bridge PTHAACEncoder *)inUserData;
...
-(aw_flv_audio_tag *)encodePCMDataToFlvTag:(NSData *)pcmData{
    ...
    if (status == noErr) {
        ...
        [self onErrorWithCode:PTEncoderErrorCodeAudioEncoderFailed des:@"aac 编码错误"];
    }
...
-(void)open{
    ...
    if (status != noErr) {
        [self onErrorWithCode:PTEncoderErrorCodeCreateAudioConverterFailed des:@"硬编码AAC创建失败"];
    }
    ...
    if (aMaxOutput == 0) {
        [self onErrorWithCode:PTEncoderErrorCodeAudioConverterGetMaxFrameSizeFailed des:@"AAC 获取最大frame size失败"];
    }
}
