...
-(aw_flv_video_tag *)encodeYUVDataToFlvTag:(NSData *)yuvData{
    ...
    if(CVPixelBufferLockBaseAddress(pixelBuf, 0) != kCVReturnSuccess){
        [self onErrorWithCode:PTEncoderErrorCodeLockSampleBaseAddressFailed des:@"encode video lock base address failed"];
        return NULL;
    }
    ...
    if (status == noErr) {
        ...
        [self onErrorWithCode:PTEncoderErrorCodeEncodeVideoFrameFailed des:@"encode video frame error"];
    }
    CVPixelBufferUnlockBaseAddress(pixelBuf, 0);
...
static void vtCompressionSessionCallback (void * CM_NULLABLE outputCallbackRefCon,
    ...
    if (status != noErr) {
        ...
        [encoder onErrorWithCode:PTEncoderErrorCodeEncodeVideoFrameFailed des:@"encode video frame error 1"];
        return;
    }
    ...
    if (!CMSampleBufferDataIsReady(sampleBuffer)) {
        dispatch_semaphore_signal(encoder.vSemaphore);
        [encoder onErrorWithCode:PTEncoderErrorCodeEncodeVideoFrameFailed des:@"encode video frame error 2"];
        return;
    }
    ...
    if (status == noErr) {
        ...
        [encoder onErrorWithCode:PTEncoderErrorCodeEncodeGetH264DataFailed des:@"got h264 data failed"];
    }
...
-(void)open{
    ...
    if (status == noErr) {
        ...
        if (status != noErr) {
            [self onErrorWithCode:PTEncoderErrorCodeVTSessionPrepareFailed des:@"硬编码vtsession prepare失败"];
        }
    }else{
        [self onErrorWithCode:PTEncoderErrorCodeVTSessionCreateFailed des:@"硬编码vtsession创建失败"];
    }
}
