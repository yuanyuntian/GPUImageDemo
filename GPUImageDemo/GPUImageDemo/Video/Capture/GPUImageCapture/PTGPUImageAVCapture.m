...
#import <libyuv-iOS/libyuv.h>
//GPUImage data handler
@interface PTGPUImageAVCaptureDataHandler : GPUImageRawDataOutput< PTGPUImageVideoCameraDelegate>
@property (nonatomic, weak) PTAVCapture *capture;
@end

@implementation PTGPUImageAVCaptureDataHandler

- (instancetype)initWithImageSize:(CGSize)newImageSize resultsInBGRAFormat:(BOOL)resultsInBGRAFormat capture:(PTAVCapture *)capture
{
...
@property (nonatomic, strong) PTGPUImageAVCaptureDataHandler *dataHandler;
...
-(void)onInit{
    //摄像头
    _videoCamera = [[PTGPUImageVideoCamera alloc] initWithSessionPreset:self.captureSessionPreset cameraPosition:AVCaptureDevicePositionFront];
    //声音
    ...
    //数据处理
    _dataHandler = [[PTGPUImageAVCaptureDataHandler alloc] initWithImageSize:CGSizeMake(self.videoConfig.width, self.videoConfig.height) resultsInBGRAFormat:YES capture:self];
    [_beautifyFilter addTarget:_dataHandler];
    _videoCamera.audioDelegate = _dataHandler;
